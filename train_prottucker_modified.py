...
class ProtTucker(nn.Module):
    def __init__(self):
        ...
        self.protTucker = nn.Sequential(
            # changed from 1024 to 1280 to fit the ESM2 embeddings
...
class CustomDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, train, datasplitter, n_classes, balanced_sampling=False):
        ...
        #     set(train.keys()))
        self.id2label, self.label2id, self.family2label = datasplitter.parse_label_mapping_families(
            set(train.keys()))
    ...
    def get_rnd_candidates(self, anchor_label, similarity_level, is_pos):
        ...
        family = anchor_label
        if similarity_level == 0: # Different family
            rnd_family = self.get_rnd_label(self.label2id.keys(), is_pos, family)
            candidates = self.label2id[rnd_family]
        elif similarity_level == 1: # Same class 
            candidates = self.label2id[family]
        else:
    ...
    def check_triplet(self, anchor_label, pos_label, neg_label, neg_hardness, pos_hardness):
        ...
        # routinary checks to assess the difference and similarity of the labels
        assert neg_hardness < pos_hardness, print(
            "Neg sample more similar than pos sample")

        assert anchor_label == pos_label, print("Pos label not overlapping:\n" +
                                                         "Diff: {}\nanchor:{}\npos:{}\nneg:{}".format(pos_hardness, anchor_label, pos_label, neg_label))
        assert anchor_label != neg_label, print("Neg label not overlapping:\n" +
...
class DataSplitter():
    def __init__(self, embedding_p, verbose=True):
        ...
        with open('family2label.tsv', 'w') as f: 
            for family, label in self.family2label.items():
                f.write('{}\t{}\n'.format(family, label))

    def get_id2embedding(self):
        return self.id2embedding

    def parse_label_mapping_families(self, id_subset):
        id2label = dict()
        label2id = dict()
        family2label = dict()

        df = pd.read_csv(self.families_label_path, sep='\t')
        i = 0
        for family in set(df['Family'].to_list()):
            family2label[family] = i
            i += 1

        for i in range(len(df.axes[0])):
            identifier = df.at[i, 'UniprotID']
            family = df.at[i, 'Family']
            if identifier not in id_subset:
                continue
            id2label[identifier] = family2label[family]
            if family in label2id.keys():
                label2id[family2label[family]].append(identifier)
            else:
                label2id[family2label[family]] = [identifier]

        if self.verbose:
            print("Total length of id2label: {}".format(len(id2label)))
        return id2label, label2id, family2label
    ...
    def get_predef_splits(self, p_train=None, p_test=None):
        ...
        if p_train is None or p_test is None:
            p_train = self.data_dir / "BacDBTF_0.5id_train3600_embeddings.h5"
            p_val = self.data_dir / "BacDBTF_val200_650M_embeddings.h5"
            p_valLookup20 = self.data_dir / "BacDBTF_0.5id_train3600_embeddings.h5"

        val = self.get_precomputed_embeddings(p_val)
        valLookup20 = self.get_precomputed_embeddings(p_valLookup20)
        train = self.get_precomputed_embeddings(p_train)
...
class MyCollator(object):
    def __call__(self, batch):
        ...
        for (anchor, pos, neg, anchor_label, pos_label, neg_label, pos_sim) in batch:
            ...
            x = torch.cat([torch.from_numpy(anchor), torch.from_numpy(pos), torch.from_numpy(neg)], dim=0)
...
class Eval():
    ...
    def preproc(self, data):
        ...
        for idx, (seq_id, embd) in enumerate(data.items()):
            ...
            if first == True: 
                dataset = torch.from_numpy(embd)
                dataset = dataset.unsqueeze(1)
                first = False
            else:
                embd = torch.from_numpy(embd).unsqueeze(1)
                dataset = torch.cat([dataset, embd], dim=-1)
    ...
    def add_sample(self, y, yhat, confmats):
        wrong = False
        if y != '':
            if not wrong and y == yhat:
                correct = 1
            else:
                correct = 0
                wrong = True
            confmats[0, correct, correct] += 1
        #the matrix is a 2x2 in which the first number is the wrong predictions and the 4th is the correct ones
        return confmats
    ...
    def evaluate(self, lookup, queries, n_nearest=1, update=True):
        ...
        for test_idx in range(n_test):  # for all test proteins
            ...
            # y = self.mask_singletons(y) # silenced because there is no family with single proteins
...
def get_baseline(test):
    ...
    print(('BASELINE\nACC-Family: {:.2f} +/- {:.2f}').format(acc[0], err[0]))
    return acc, err
...
def main():
    ...
    embedding_p = data_dir / "BacDBTF" /"BacDBTF_morethan50_650M_embeddings.h5" # path to your pre-computed embeddings. IDs have to align with label IDs
    print("Loading dataset from: {}".format(embedding_p))

    # give your experiment a meaningful name here
    experiment_name = "BacDBTF50_ESM2"
    ...
    n_classes = 1  # number of class-lvls; makes it easier to adjust for other problems
